...
// ── Set API key ──
// @ts-ignore
server.tool('mica_set_api_key', { key: z.string() }, async ({ key }) => {
...
// ── Route compute ──
// @ts-ignore
server.tool('mica_route_compute', 'Route a compute-heavy task through MVM nodes running on cheap renewable energy. Saves tokens by offloading inference, batch processing, and long-running jobs to distributed nodes.', {
...
// ── Estimate savings ──
// @ts-ignore
server.tool('mica_estimate_savings', 'Estimate token and cost savings for a workload routed through MVM versus standard cloud compute.', {
...
// ── Node status ──
// @ts-ignore
server.tool('mica_node_status', 'Check the current status of MVM node clusters, including availability, energy costs, and load.', {}, async () => {
    const guard = requireKey();
    if (guard)
        return guard;
    return jsonResult({
...
// ── Check job status ──
// @ts-ignore
server.tool('mica_check_job', 'Check the status of a previously submitted MVM compute job.', {
